...
public class MultiplayerVRSynchronization : MonoBehaviour, IPunObservable
{
    ...
    [Header("Hand Tracking Transform")]
    public Transform _leftHandTransform;
    public Transform _rightHandTransform;
    
    private Vector3 localLeftHandPosition;
    private Quaternion localLeftHandRotation;
    private Vector3 localRightHandPosition;
    private Quaternion localRightHandRotation;
    ...
    public void Awake()
    {
        ...
        //Left Hand Tracking Init
        m_StoredPosition_LeftHand = _leftHandTransform.localPosition;
        m_NetworkPosition_LeftHand = Vector3.zero;
        m_NetworkRotation_LeftHand = Quaternion.identity;
        
        //Right Hand Tracking Init
        m_StoredPosition_RightHand = _rightHandTransform.localPosition;
        m_NetworkPosition_RightHand = Vector3.zero;
        m_NetworkRotation_RightHand = Quaternion.identity;
    }
    ...
    public void Update()
    {
        if (!this.m_PhotonView.IsMine)
        {
            ...
            _leftHandTransform.localPosition = Vector3.MoveTowards(_leftHandTransform.localPosition, this.m_NetworkPosition_LeftHand, this.m_Distance_LeftHand * (1.0f / PhotonNetwork.SerializationRate));
            _leftHandTransform.localRotation = Quaternion.RotateTowards(_leftHandTransform.localRotation, this.m_NetworkRotation_LeftHand, this.m_Angle_LeftHand * (1.0f / PhotonNetwork.SerializationRate));
            _rightHandTransform.localPosition = Vector3.MoveTowards(_rightHandTransform.localPosition, this.m_NetworkPosition_RightHand, this.m_Distance_RightHand * (1.0f / PhotonNetwork.SerializationRate));
            _rightHandTransform.localRotation = Quaternion.RotateTowards(_rightHandTransform.localRotation, this.m_NetworkRotation_RightHand, this.m_Angle_RightHand * (1.0f / PhotonNetwork.SerializationRate));
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.IsWriting)
        {
            ...
            //Left Hand Tracking
            //Send Left Hand position data
            this.m_Direction_LeftHand = _leftHandTransform.localPosition - this.m_StoredPosition_LeftHand;
            this.m_StoredPosition_LeftHand = _leftHandTransform.localPosition;
            
            stream.SendNext(_leftHandTransform.localPosition);
            stream.SendNext(this.m_Direction_LeftHand);
            
            //Right Hand Tracking
            //Send Right Hand position data
            this.m_Direction_RightHand = _rightHandTransform.localPosition - this.m_StoredPosition_RightHand;
            this.m_StoredPosition_RightHand = _rightHandTransform.localPosition;
            
            stream.SendNext(_rightHandTransform.localPosition);
            stream.SendNext(this.m_Direction_RightHand);
